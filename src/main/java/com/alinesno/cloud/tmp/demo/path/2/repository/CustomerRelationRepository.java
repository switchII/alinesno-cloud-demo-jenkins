package com.alinesno.cloud.tmp.demo.path.2.repository;

import com.alinesno.cloud.tmp.demo.path.2.entity.CustomerRelationEntity;
import com.alinesno.cloud.common.core.orm.repository.IBaseJpaRepository;

/**
 * <p>
  * 客户关系表 持久层接口
 * </p>
 *
 * @author 罗小东
 * @since 2019-09-15 20:52:37
 */
public interface CustomerRelationRepository extends IBaseJpaRepository<CustomerRelationEntity, String> {

}
